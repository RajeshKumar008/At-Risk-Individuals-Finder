...
# Add path to your face recognition models
sys.path.append('C:\\Users\\mfahe\\missing_person_finder\\face_recognition_models-master')
...
app.config['MAIL_PASSWORD'] = 'zvvu mdob oezo ptqn'  # Ensure to use environment variables in production
mail = Mail(app)
...
def found_person():
    if request.method == 'POST':
        ...
        # Call the recognizer to check for a match
...
def check_for_match(image_path):
    ...
    for person in all_complaints:
        ...
        # Compare found person encoding with DB encoding
        ...
        print(f"Comparing {person.name} with found image: Match = {results[0]}, Distance = {face_distances[0]}")
